...
export async function scraper(): Promise<CinemaShowing[]> {
        ...
        BASE_URL + '/search_film_programmes/'
    );
    ...
    const scriptTag = root.querySelectorAll("script").find((s) =>
    s.innerText.includes("var shows")
    );
    ...
    for (const show of parsedJson) {
        ...
        const existingFilm = filmShowings.find(f => f.film.title === show.title && f.film.url === url)
        
        const startTime =  DateTime.fromFormat(show.show_time, "yyyy-MM-dd HH:mm:ss").toISO()?.toString()
        if (startTime === undefined) {
            console.error(`${LOG_PREFIX} Failed to parse starttime from ${show.show_time}`);
            continue
        }
        const showing = {
                startTime,
            ...
            filmShowings.map(async (filmShowing) => {
                ...
                try {
                    ...
                    const info = root
                        ...
                        .map((e) => e.innerText.replace(filmShowing.film.title, '').trim());

                    const imgSrc = root.querySelectorAll('img').map(e => e.getAttribute('src')).filter(src => src?.includes('/download_file/view_inline')).at(0);
