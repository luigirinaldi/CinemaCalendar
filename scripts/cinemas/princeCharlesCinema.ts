...
const months: Record<string, number> = [
    ...
    (acc, month, index) => ({ ...acc, [month]: index }),
...
function scrape(page: any, callback: (s: ScrapedShowing) => void) {
    page.querySelectorAll('.jacrofilm-list .jacro-event').forEach(
        (eventEl: Element) => {
            const title = (
                eventEl.querySelector('.liveeventtitle')?.textContent || ''
            ).trim();
            const filmUrl = (
            ...
            const countryText = metaSpans.find((s) =>
                /^[A-Z]{2,}(?:\s+[A-Z]+)*$/.test(s)
            );
            ...
            const filmInfoText = (
                eventEl.querySelector('.film-info span')?.textContent || ''
            ).trim();
            const filmDirector =
                filmInfoText.match(/Directed by\s*(.*)/i)?.[1] || null;
    ...
    scrape(
        ...
        ({
            ...
            if (
                ...
                entry.film.duration = duration;
            if (filmDirector && !entry.film.director)
                entry.film.director = filmDirector;
            if (typeof filmYear === 'number' && !entry.film.year)
                entry.film.year = filmYear;
            if (filmCountry && !entry.film.country)
                entry.film.country = filmCountry;
            if (coverUrl && !entry.film.coverUrl)
                entry.film.coverUrl = coverUrl;
