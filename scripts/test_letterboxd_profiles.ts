...
console.log(
    `Mode: ${isFullMode ? 'full' : 'quick'} (maxPages=${isFullMode ? '∞' : maxPages})\n`
);
...
for (const { username, minFilms } of PROFILES) {
    ...
    try {
        ...
        pass(
            films.length >= minFilms,
            `${films.length} films >= ${minFilms} minimum`
        );
        pass(
            films.every((f) => f.slug.length > 0),
            'every film has a non-empty slug'
        );
        pass(
            films.every((f) => f.title.length > 0),
            'every film has a non-empty title'
        );
    } catch (e) {
