...
export default function ByCinemaView({
    screenings,
    getMovie,
    getCinema,
}: ByCinemaViewProps) {
        ...
        <div className="space-y-4">
            {Object.entries(groupByCinema(screenings)).map(
                ([cinemaId, cinemaScreenings]) => {
                    ...
                    const movieGroups = groupByMovie(
                        cinemaScreenings,
                        getMovie
                        ...
                        >
                            ...
                            >
                                ...
                                <span className="text-sm text-neutral-400 bg-neutral-700 px-2 py-0.5 rounded-full">
                                    {movieGroups.length} film
                                    {movieGroups.length !== 1 ? 's' : ''}
                                </span>
                            </button>
                            {!isCollapsed && (
                                    ...
                                    {movieGroups.map(
                                        ([key, movieScreenings]) => {
                                            const movie = getMovie(
                                                movieScreenings[0]?.film_id ??
                                                    -1
                                            );
                                            const byDay = Object.entries(
                                                groupByDay(
                                                    movieScreenings.sort(
                                                        sortScreeningByStartTime
                                                    )
                                                )
                                            ).sort(sortGroupedByStartTime);
