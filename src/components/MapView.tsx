...
async function fetchOsmCinemas(
    areaId: number,
    signal: AbortSignal
): Promise<OsmCinema[]> {
...
export default function MapView({
    ...
    useEffect(() => {
        ...
        fetch(url, { signal: controller.signal })
            ...
            .catch((err) => {
                if (err.name !== 'AbortError') setBoundaryGeojson(null);
            });
    ...
    const trackedWithCoords = cinemas
        ...
        .flatMap((c) => {
            const coords = parseCinemaCoords(c);
            return coords ? [{ cinema: c, coords }] : [];
        });
    ...
    function matchTracked(osm: OsmCinema): CinemaTable | null {
        ...
        for (const { cinema, coords } of trackedWithCoords) {
            const distSq =
                (osm.lat - coords.lat) ** 2 + (osm.lng - coords.lng) ** 2;
    ...
    const osmMatchedCinemaIds = new Set(
        osmCinemas
            .map((osm) => matchTracked(osm)?.id)
            .filter((id): id is number => id != null)
    );
    const unmatchedTracked = trackedWithCoords.filter(
        ({ cinema }) => !osmMatchedCinemaIds.has(cinema.id)
    );
    ...
    function renderMarker(
        ...
        return (
            <Marker
                ...
                position={position}
                icon={icon}
                zIndexOffset={available ? 1000 : 100}
            >
        ...
        >
            ...
            >
                ...
                {[...osmCinemas]
                    .map((osm) => {
                        ...
                        const available =
                            tracked != null && activeCinemaIds.has(tracked.id);
                        const count = tracked
                            ? (cinemaMovieCounts.get(tracked.id) ?? 0)
                            : 0;
                                ...
                                {tracked ? (
                                    count > 0 ? (
                                        ...
                                        <p className="text-neutral-400 text-xs mt-0.5">
                                            No screenings in current range
                                        </p>
                                    )
                                ) : osm.website ? (
                                    <a
                                        ...
                                        className="text-blue-500 text-xs hover:underline break-all"
                                    >
                                        {osm.website}
                                    </a>
                                ) : (
            ...
            {trackedUnmapped.length > 0 && (
                <div className="absolute top-4 right-4 z-[1000] bg-neutral-900/90 border border-neutral-700 rounded-lg p-3 text-xs text-white max-w-48">
                    <p className="font-semibold mb-1 text-neutral-400">
                        Missing coordinates:
                    </p>
